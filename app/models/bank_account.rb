class BankAccount < ApplicationRecord
  ...
  def find_import_method
    ...
    return method(:import_from_meinelba) if /^AT\d{2}3\d{15}$/.match(iban)
  ...
  def import_from_meinelba(bank_account)
    meinelba_config = FoodsoftConfig[:meinelba]
    raise "meinelba configuration missing" if not meinelba_config

    mandant = meinelba_config[:mandant].to_s
    username = meinelba_config[:username]
    password = meinelba_config[:password]
    iban = bank_account.iban

    count = 0
    continuation_point = nil

    MeinELBA.login(mandant, username, password) do |h|
      bank_account.balance = h.balance(iban)
      continuation_point = h.transactions(iban, bank_account.import_continuation_point || '') do |t|
        bank_account.bank_transactions.where(:external_id => t[:id]).first_or_create.update(
          :date => t[:date],
